...
export function ProductRow({ product }: { product: AdminProductRow }) {
  ...
  return (
    <motion.li
      ...
      className="flex items-center gap-3 rounded-2xl border border-border bg-surface p-3 transition-colors hover:border-border/80 hover:bg-surface-2/40"
    >
      ...
      <div className="min-w-0 flex-1">
        {/* Nombre prominente, en su propia línea */}
          ...
          {confirming ? (
            <>
              <button
                ...
                disabled={pending}
                className="rounded-lg bg-danger/15 px-2 py-1 text-xs font-semibold text-danger"
              >
                Confirmar
              </button>
              <button
